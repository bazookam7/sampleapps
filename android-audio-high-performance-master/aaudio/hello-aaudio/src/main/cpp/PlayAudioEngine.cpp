...
aaudio_data_callback_result_t dataCallback1(AAudioStream *stream, void *userData,
                                           void *audioData, int32_t numFrames) {
  assert(userData && audioData);
  PlayAudioEngine *audioEngine = reinterpret_cast<PlayAudioEngine *>(userData);
...
void PlayAudioEngine::prepareOscillators1() {
...
void PlayAudioEngine::setupPlaybackStreamParameters1(AAudioStreamBuilder *builder) {
  AAudioStreamBuilder_setDeviceId(builder, playbackDeviceId_);
  AAudioStreamBuilder_setFormat(builder, sampleFormat_);
  AAudioStreamBuilder_setChannelCount(builder, sampleChannels_);
  ...
  AAudioStreamBuilder_setPerformanceMode(builder, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
  AAudioStreamBuilder_setDirection(builder, AAUDIO_DIRECTION_OUTPUT);
  AAudioStreamBuilder_setDataCallback(builder, ::dataCallback1, this);
  AAudioStreamBuilder_setErrorCallback(builder, ::errorCallback1, this);
    AAudioStreamBuilder_setBufferCapacityInFrames(builder, 10000);
}
...
//  assert(stream == playStream_);
  ...
  aaudio_data_callback_result_t PlayAudioEngine::dataCallback1(AAudioStream *stream,
                                                              void *audioData,
                                                              int32_t numFrames) {
//    assert(stream == playStream_);
...
//  assert(stream == playStream_);
  ...
  void PlayAudioEngine::errorCallback1(AAudioStream *stream,
                                      aaudio_result_t error){
...
void PlayAudioEngine::setBufferSizeInBursts(int32_t numBursts) {
  ...
  PlayAudioEngine::bufferSizeSelection_1 = numBursts;
}
